...
const SimulationDetail: React.FunctionComponent<SimulationDetailProps> = () => {
    const methods = useForm();
    const router = useRouter();
    const [popData, setPopData] = React.useState<Question>();
    ...
    const _handleOnPopUp = (question: Question) => {
        if (question) {
            setPopData(question);
            setPop(true);
            return;
    ...
    return (
        <>
                                            ...
                                            questions.map((question, index) => (
                                                <tr key={question.id}>
                                                    ...
                                                    <td className="relative py-4 pl-3 pr-4 text-sm font-medium text-right whitespace-nowrap sm:pr-6">
                                                        <button
                                                            onClick={() => _handleOnPopUp(question)}
            ...
            <div
                className={`absolute cursor-pointer top-0 left-0 flex items-center justify-center w-full h-full text-white bg-gray-700/60 ${
                    pop ? 'flex' : 'hidden'
                }`}
